...
#cell_responses = []
#w = csv.writer(open("cell_responses_cut_new_CORTICAL.csv", "w")) #file location to save responses to
...
for cell_no in range(len(mu_cells)):
    ...
    k_max_id = 50
    feedback_strength = np.array([0,-0.6])#, 0.5, 0.8 - varies cortical feedback strength values if non-zero
    response = np.zeros([k_max_id, len(feedback_strength)]) /pq.s
    
    print(X.g1_xvar,X.g1_yvar,'       ',X.g2_xvar,X.g2_yvar)
    '''
    for j, d in enumerate(feedback_strength):
        ...
        # create integrator
        integrator = network.create_integrator(nt=5, nr=7, dt=1*pq.ms, dr=1*pq.deg)#nt=8, nr=9, dt=1*pq.ms, dr=0.1*pq.deg
        ...
        #Kdg_s = kernel.spatial.create_dog_ft(A=1, a=5*pq.deg, B=0.2, b=10*pq.deg)#A=1, a=0.62*pq.deg, B=0.85, b=1.26*pq.deg
        Kg_s = kernel.spatial.create_gauss_ft(A=1, a=0.1*pq.deg)
        Kg_s1 = kernel.spatial.create_gauss_ft(A=1, a=0.3*pq.deg)
        Kg_s2 = kernel.spatial.create_gauss_ft(A=1, a=4*pq.deg)
        ...
        for i, k_d in enumerate(spatial_angular_freqs):
            ...
            stimulus = pylgn.stimulus.create_fullfield_grating_ft(wavenumber=k_d, contrast=1)#create_fullfield_grating_ft(wavenumber=k_d, contrast=1)
