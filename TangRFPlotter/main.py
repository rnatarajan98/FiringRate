...
#functions
def analyse(content):
    cell = cells[content[0]]
    parameter = cell.sf_tuning[:,0]
    tuning = cell.sf_tuning[:,1]
    tuningSEM = cell.sf_tuning[:,2]
    parameterfit = cell.sf_curvefit[:,0]
    tuningfit = cell.sf_curvefit[:,1]
    spont = [cell.Spont_mean, cell.Spont_mean]
    spontSEM = [cell.Spont_SEM, cell.Spont_SEM]
    fig = plt.figure()
    plt.subplot(1, 2, 1)
	#Plot the receptive field using matplotlib
    rfield = cell.rf_sta_lat.tolist()
...
x = loadmat('C:\\Uni\\Neuro\\Receptive_Field_Plotter\\cells.mat')
...
btn.pack(side=TOP)
cell_list = Listbox(navigationframe)
i=1
for cell in cells:
